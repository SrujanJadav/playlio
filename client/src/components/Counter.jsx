...
export default function Counter({
  ...
  const valString = (value ?? 0).toString();
  const calculatedPlaces = places || [...valString].map((ch, i, a) => {
    if (ch === '.') {
      return '.';
    } else {
      return (
    ...
    <span className="counter-container" style={containerStyle}>
      <span className="counter-counter" style={{ ...defaultCounterStyle, ...counterStyle }}>
        {calculatedPlaces.map((place, idx) => (
          <Digit key={`${place}-${idx}`} place={place} value={value ?? 0} height={height} digitStyle={digitStyle} />
        ))}
      </span>
